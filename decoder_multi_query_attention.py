...
class TransformerBlock(nn.Module):
    def __init__(self, max_len:int, emb_dim: int, ffn_dim: int, num_heads: int, dropout: float=0.1, act: nn.Module=nn.ReLU()):
        ...
        self.v = nn.Linear(int(emb_dim/num_heads), int(emb_dim/num_heads))
        self.q = nn.Linear(int(emb_dim/num_heads), int(emb_dim/num_heads))
    ...
    def attention(self, x: torch.tensor) -> torch.tensor:
        ...
        x_tmp = einops.rearrange(x, "B N (E H) -> B H N E", H=self.num_heads)
